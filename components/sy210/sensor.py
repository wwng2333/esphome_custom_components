...
SY210Component = sy210_ns.class_(
    "SY210Component", uart.UARTDevice, cg.PollingComponent
)
...
CONFIG_SCHEMA = cv.All(
    cv.Schema(
        {
            cv.GenerateID(): cv.declare_id(SY210Component),
...
def validate_interval_uart(config):
    ...
    uart.final_validate_device_schema(
        "sy210", baud_rate=9600, require_rx=True, require_tx=require_tx
    )(config)
